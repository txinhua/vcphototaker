...
@interface VCPTSlider () {
    CGFloat gap;
    CGFloat inverseGap;
}

@property (nonatomic, assign) VCPTSliderDirection direction;//方向（横或竖）
...
@implementation VCPTSlider

- (id)initWithFrame:(CGRect)frame
{
    return [self initWithFrame:frame direction:VCPTSliderDirectionHorizonal];
}
 ...
 *  @return SCSlider
 */
- (id)initWithFrame:(CGRect)frame direction:(VCPTSliderDirection)direction {
...
- (void)drawRect:(CGRect)rect {
    ...
    CGFloat startLineX = (_direction == VCPTSliderDirectionHorizonal ? gap : (self.frame.size.width - _lineWidth) / 2);
    CGFloat startLineY = (_direction == VCPTSliderDirectionHorizonal ? (self.frame.size.height - _lineWidth) / 2 : gap);
    
    CGFloat endLineX = (_direction == VCPTSliderDirectionHorizonal ? self.frame.size.width - gap : (self.frame.size.width - _lineWidth) / 2);
    CGFloat endLineY = (_direction == VCPTSliderDirectionHorizonal ? (self.frame.size.height - _lineWidth) / 2 : self.frame.size.height- gap);
    ...
    CGContextSetLineWidth(context, _lineWidth);//线的宽度
    CGFloat slidedLineX = (_direction == VCPTSliderDirectionHorizonal ? MAX(gap, (_scaleNum * self.frame.size.width - gap)) : startLineX);
    CGFloat slidedLineY = (_direction == VCPTSliderDirectionHorizonal ? startLineY : MAX(gap, (_scaleNum * self.frame.size.height - gap)));
    CGContextMoveToPoint(context, startLineX, startLineY);//起点
    ...
    CGFloat circleX = (_direction == VCPTSliderDirectionHorizonal ? MAX(_circleRadius + penWidth, slidedLineX - penWidth - inverseGap) : startLineX);
    CGFloat circleY = (_direction == VCPTSliderDirectionHorizonal ? startLineY : MAX(_circleRadius + penWidth, slidedLineY - penWidth - inverseGap));
...
- (void)updateTouchPoint:(NSSet*)touches {
    ...
    self.scaleNum = (_direction == VCPTSliderDirectionHorizonal ? touchPoint.x : touchPoint.y) / (_direction == VCPTSliderDirectionHorizonal ? self.frame.size.width : self.frame.size.height);
}
